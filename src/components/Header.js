...
const Header = () => {
 ...
 useEffect(()=>{
  ...
  className='bg-pink-200/20 fixed w-full max-w-[1800px]
  z-50 py-4
  '>
     <motion.div variants={staggerContainer(0.3 , 0.5)} 
     initial='hidden'
     animate={"show"}
     className="container mx-auto">
      ...
      className={`${nav ? 'gap-y-0' : 'gap-y-2'} flex flex-col items-center
       justify-center w-12 h-12 p-3 order-2 lg:border-none
      ...
      >
        <a href="/">
          <img className={`${isActive ? 'w-[90px] h-[90px]' :
           'w-107px h-107px'}`} src={LogoWhite} alt="logo" />
        </a>
      </motion.div>
        {/* social icons */}
        <motion.div
        ...
        className='hidden lg:flex ' >
          ...
          <motion.div 
           ...
           initial="hidden"
           animate= {nav ? 'show' : ""}
          className='absolute bg-accent w-[310px] h-[50vh] right-0
